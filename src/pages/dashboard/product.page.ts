...
export class ProductPage extends DashboardPage {
    ...
    async createProduct(productName: string, productPrice: string, productOptionName: string, productVariants: Array<string>) {
        await this.page.locator("//input[@placeholder='Short Sleeve T-Shirt']").fill(productName);
        await this.page.locator("//input[@id='price']").fill(productPrice);
        await this.page.click("//a[@class='pull-right']");
        await this.page.locator("//input[@id='option-name']").fill(productOptionName);
        await this.page.locator("//input[@placeholder='Separate options with comma']").fill(productVariants.join(","));
        await this.page.keyboard.press('Enter');
        await this.page.click("//span[normalize-space()='Save product']");
    }

    async verifyProduct(context, productName: string, productPrice: string, productOptionName: string, productVariants: Array<string>) {
